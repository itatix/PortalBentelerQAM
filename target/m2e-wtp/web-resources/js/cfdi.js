...
Ext.onReady(function(){
	...
	bundle.onReady(function(){
		...
		var btnSelectDoc = new Ext.Button({
			text: bundle.getMsg('label.selectDoc'),
...
//					       referenciaField,
	...
	function windowSelection(){
		...
		firstDate = new Date(firstDate.getFullYear() - 1,firstDate.getMonth(),1);
		...
		var btnAgregarDoc = new Ext.Button({
			...
			handler: function(){
				...
				Ext.getCmp('hdnReceptionType').setValue(Ext.getCmp('cmbReceptionTypeField').getValue());
...
//			          }
		...
		var winSelect = Ext.getCmp('winSelection') || new Ext.Window({
			id: 'winSelection',
			title: bundle.getMsg('label.selectDoc'),
			frame: true,
			layout: 'form',
			closeAction: 'hide',
			width: 1050,
