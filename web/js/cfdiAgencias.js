...
//                			} else {
                    ...
                    if(rowIndex === 2 || rowIndex === 3) {
                    		roweditor.grid.getColumnModel().getColumnAt(2).getEditor(rowIndex).disable();
                    		roweditor.grid.getColumnModel().getColumnAt(3).getEditor(rowIndex).disable();
                    		roweditor.grid.getColumnModel().getColumnAt(4).getEditor(rowIndex).disable();
                    		roweditor.grid.getColumnModel().getColumnAt(5).getEditor(rowIndex).disable();
                    		roweditor.grid.getColumnModel().getColumnAt(6).getEditor(rowIndex).disable();
...
//                    if(rowIndex == 0) {
//                    		console.log(record);
//                    		if(record.get('orderid') && record.get('costcenter')){
//                    			Ext.Msg.alert('Atención','Solo debe proporcionar una Orden Interna o un Centro de Costo o un Proyecto');
//                    			return false;
//                    		}
//                    }
                },
...
//        cuentaMayorEditor.on('beforeedit', function(editor, e) {
//        		if(this.grid.getStore().getAt(e).get('orderid').length > 0) {
//        			grdcuentaMayor.getPlugin('rowEditing').editor.form.findField('costcenter').disable();
//        			grdcuentaMayor.getPlugin('rowEditing').editor.form.findField('project').disable();
//        		}
//        });
        //********************************************** End Components **************************************************/
...
//      });
        ...
        var formCFDI = new Ext.form.FormPanel({
            title: bundle.getMsg('label.wopcfdiAgencias'),
            id: 'cfdiForm',
            fileUpload: true,
            method: 'POST',
            ...
            Ext.Ajax.request({
                   ...
                   success: function(result,request) {
                        if(result.responseText.indexOf("login")>=0) {
                            ...
                            frm.submit({
                                url: contextrootpath + '/soc/addCFDIAgencias.action',
