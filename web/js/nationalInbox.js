		...
		var tipoTransporteCmb = new Ext.form.ComboBox({
			...
			listeners: {
				'select': function(c,r,i) {
					...
					var newDisplayValue = this.getRawValue().replace(/&eacute;/g,"é").replace(/&iacute;/g,"í");
				...
				Ext.Ajax.request({
					   ...
					   success: function(result,request) {
							if(result.responseText.indexOf("login")>=0) {
								...
								if(tickets.length <= 0) {
									Ext.Msg.alert('Error','Debe seleccionar al menos un ticket');
									return;
								}
								Ext.getCmp('tickets').setValue(Ext.encode(tickets));
								Ext.getCmp('tipoFac').setValue("1");
								frm.submit({
